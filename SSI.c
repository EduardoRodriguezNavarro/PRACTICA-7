...
extern void Configurar_SSI2(void)
{
    ...
    GPIOD_AHB->AFSEL = (1<<3)|(1<<2)|(1<<1)|(1<<0); //antes del 4 al 7.LOS CUALES SERAN CONTROLADOS POR EL SSI
    ...
    SSI2->CC = (0x0<<0); //HABILITA system clock = 50MHz
...
//____________________________________________________________SPI CONFIG
...
*/
extern void DacSl (unsigned int Dac_V)
{
    ...
    string = (Dac_V >> 8 ) & 0x0F;//LE MANDO LA PRIMER MITAD  (8 BITS) EN ESTADO ALTO
    
    string |=0x30;
    
    SPI_write (string);//LE MANDO LA SEGUNDA MITAD EN ESTADO BAJO
